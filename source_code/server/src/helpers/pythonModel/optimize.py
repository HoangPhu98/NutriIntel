...
def solveModelMin(food, conditionNutri):
    ...
    #Variables
    weight = [solver.NumVar(0, solver.infinity(), 'w-' + food[i][0])
              for i in range(nbFood)]

    #Constraints
    constraint = [solver.Constraint(
        conditionNutri[i][1], solver.infinity()) for i in range(nbNutri)]
...
def solveProblem(food, conditionNutri):
    ...
    #Constraints
    constraint = [solver.Constraint(
        conditionNutri[i][1], conditionNutri[i][2]) for i in range(numNutri)]
    ...
    for i in range(len(food)):
        for j in range(len(food[i])):
            if not(type(food[i][j]) is int):
                food[i][j] = unicodedata.normalize(
                    'NFKD', food[i][j]).encode('ascii', 'ignore')

    for i in range(len(condition)):
        for j in range(len(condition[i])):
            if not(type(condition[i][j]) is int):
                condition[i][j] = unicodedata.normalize(
                    'NFKD', condition[i][j]).encode('ascii', 'ignore')
